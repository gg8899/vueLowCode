...
const isDrawerShown = ref<DrawerMode>(null);


const toggleDrawer = (mode: DrawerMode) => {
    if (mode === isDrawerShown.value) {
        isDrawerShown.value = null;
        return; // 相同位置点击两次收起侧边栏
    }
    isDrawerShown.value = mode;
}
...
<template>
    <div class="app-left-panel-wrapper">
        <div class="app-left-panel-bar" :style="{ boxShadow: 'var(--color-gray-300) 1px 0px 0px' }">
            <div :class="['app-left-panel-item', isDrawerShown === 'outline' && 'active']"
                @click="toggleDrawer('outline')">
                <Page size="20" :style="{ lineHeight: 0.7 }" />
            </div>
            <div :class="['app-left-panel-item', isDrawerShown === 'blocks' && 'active']"
